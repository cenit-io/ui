...
export default function RefManyControl({
  ...
  if (aValue) {
    if (open) {
      dropButton = aValue.length > 0 &&
        <IconButton onClick={() => setOpen(false)} size="large"><ArrowDropUpIcon /></IconButton>;
    ...
    if (!readOnly && !disabled) {
      deleteButton = <IconButton onClick={handleClear} disabled={disabled} size="large"><ClearIcon /></IconButton>;
    }
  }
  ...
  if (!readOnly && !disabled) {
    ...
    addButton = <IconButton onClick={addNew} disabled={disabled} size="large"><AddNewIcon /></IconButton>;
  }
