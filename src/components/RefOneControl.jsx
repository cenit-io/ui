  ...
  if (!disabled) {
    if (value.get()) {
      editButton = !editDisabled && (
        <IconButton onClick={handleEdit} disabled={disabled} size="large">
          <EditIcon />
        </IconButton>
      );
      if (!deleteDisabled && !readOnly) {
        deleteButton = (
          <IconButton onClick={handleClear} disabled={disabled} size="large">
            <ClearIcon />
          </IconButton>
        );
      }
    ...
    if (!readOnly && !addDisabled) {
      addButton = (
        <IconButton onClick={handleAddNew} disabled={disabled} size="large">
          <AddIcon />
        </IconButton>
      );
    }
