...
function EmbedsOneControl({
  ...
  width, disabled, onStack, readOnly, fetched, config
}) {
  ...
  if (value.get()) {
    if (open) {
      ...
      actionButton =
        <IconButton onClick={() => setOpen(true)} disabled={disabled} size="large"><ArrowDropDownIcon /></IconButton>;
    }
    if (!readOnly) {
      deleteButton = <IconButton onClick={handleDelete} disabled={disabled} size="large"><ClearIcon /></IconButton>;
    }
  } else if (!readOnly) {
    actionButton = <IconButton onClick={addNew} disabled={disabled} size="large"><AddIcon /></IconButton>;
  }
