...
const isAppShellVisible = async () => {
  ...
  return hasBanner || hasAvatar || hasNav || hasMenuHeading || hasDocumentTypes || hasDocumentTypesText || hasRecent;
};
...
try {
  ...
  const hasUiRootContent = await page.locator('#root > *').count().then((n) => n > 0).catch(() => false);
  const hasAuthenticatedContext = (await isAppShellVisible()) || isServerAppPage() || hasOauthCallbackCode() || (page.url().startsWith(uiUrl) && hasUiRootContent);
