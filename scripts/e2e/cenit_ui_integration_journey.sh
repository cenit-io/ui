...
CENIT_ROOT="${CENIT_ROOT:-$ROOT_DIR/../cenit}"
COMPOSE_FILE="${CENIT_COMPOSE_FILE:-$CENIT_ROOT/docker-compose.yml}"
...
CENIT_E2E_AUTH_PREFLIGHT="${CENIT_E2E_AUTH_PREFLIGHT:-1}"
...
CENIT_API_INTEGRATION_JOURNEY_SCRIPT="${CENIT_API_INTEGRATION_JOURNEY_SCRIPT:-$CENIT_ROOT/scripts/e2e/cenit_api_integration_journey.sh}"
...
echo "  CENIT_E2E_API_PRECHECK=$CENIT_E2E_API_PRECHECK"
...
if [[ "$CENIT_E2E_API_PRECHECK" == "1" ]]; then
  if [[ -x "$CENIT_API_INTEGRATION_JOURNEY_SCRIPT" ]]; then
    echo "Running backend API integration-journey precheck: $CENIT_API_INTEGRATION_JOURNEY_SCRIPT"
    "$CENIT_API_INTEGRATION_JOURNEY_SCRIPT"
  else
    echo "Backend API integration-journey precheck script is missing or not executable: $CENIT_API_INTEGRATION_JOURNEY_SCRIPT" >&2
    exit 1
  fi
fi
