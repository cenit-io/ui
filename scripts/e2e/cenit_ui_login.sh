...
CENIT_API_LOGIN_SCRIPT="${CENIT_API_LOGIN_SCRIPT:-$CENIT_ROOT/scripts/e2e/cenit_api_login.sh}"
...
if [[ "$CENIT_E2E_API_PRECHECK" == "1" ]]; then
  if [[ -x "$CENIT_API_LOGIN_SCRIPT" ]]; then
    echo "Running backend API login precheck: $CENIT_API_LOGIN_SCRIPT"
    "$CENIT_API_LOGIN_SCRIPT"
  else
    echo "Backend API login precheck script is missing or not executable: $CENIT_API_LOGIN_SCRIPT" >&2
    exit 1
  fi
fi
